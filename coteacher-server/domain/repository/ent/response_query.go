...
func (rq *ResponseQuery) QueryStudent() *StudentQuery {
	query := (&StudentClient{config: rq.config}).Query()
...
func (rq *ResponseQuery) QueryForm() *FormQuery {
	query := (&FormClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *ResponseQuery) QueryAnswer() *AnswerQuery {
	query := (&AnswerClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *ResponseQuery) WithStudent(opts ...func(*StudentQuery)) *ResponseQuery {
	query := (&StudentClient{config: rq.config}).Query()
...
func (rq *ResponseQuery) WithForm(opts ...func(*FormQuery)) *ResponseQuery {
	query := (&FormClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(response.FieldStudentID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Response.Query().
//		Select(response.FieldStudentID).
//		Scan(ctx, &v)
...
func (rq *ResponseQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Response, error) {
	...
	if query := rq.withStudent; query != nil {
		if err := rq.loadStudent(ctx, query, nodes, nil,
			func(n *Response, e *Student) { n.Edges.Student = e }); err != nil {
			return nil, err
		}
	}
	if query := rq.withForm; query != nil {
		if err := rq.loadForm(ctx, query, nodes, nil,
			func(n *Response, e *Form) { n.Edges.Form = e }); err != nil {
			return nil, err
		}
	}
	if query := rq.withAnswer; query != nil {
		if err := rq.loadAnswer(ctx, query, nodes,
			func(n *Response) { n.Edges.Answer = []*Answer{} },
			func(n *Response, e *Answer) { n.Edges.Answer = append(n.Edges.Answer, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ResponseQuery) loadStudent(ctx context.Context, query *StudentQuery, nodes []*Response, init func(*Response), assign func(*Response, *Student)) error {
...
func (rq *ResponseQuery) loadForm(ctx context.Context, query *FormQuery, nodes []*Response, init func(*Response), assign func(*Response, *Form)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Response)
	for i := range nodes {
		fk := nodes[i].FormID
...
func (rq *ResponseQuery) loadAnswer(ctx context.Context, query *AnswerQuery, nodes []*Response, init func(*Response), assign func(*Response, *Answer)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(answer.FieldResponseID)
	}
	query.Where(predicate.Answer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(response.AnswerColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "response_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (rq *ResponseQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := rq.ctx.Fields; len(fields) > 0 {
		...
		if rq.withStudent != nil {
			_spec.Node.AddColumnOnce(response.FieldStudentID)
		}
		if rq.withForm != nil {
			_spec.Node.AddColumnOnce(response.FieldFormID)
		}
	}
