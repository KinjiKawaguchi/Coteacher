...
// QueryStudentClasses chains the current query on the "studentClasses" edge.
func (sq *StudentQuery) QueryStudentClasses() *StudentClassQuery {
	query := (&StudentClassClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *StudentQuery) IDs(ctx context.Context) (ids []string, err error) {
...
// the "studentClasses" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *StudentQuery) WithStudentClasses(opts ...func(*StudentClassQuery)) *StudentQuery {
	query := (&StudentClassClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *StudentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Student, error) {
	...
	if query := sq.withStudentClasses; query != nil {
...
func (sq *StudentQuery) loadStudentClasses(ctx context.Context, query *StudentClassQuery, nodes []*Student, init func(*Student), assign func(*Student, *StudentClass)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(studentclass.FieldStudentID)
	}
...
func (sq *StudentQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(student.Table, student.Columns, sqlgraph.NewFieldSpec(student.FieldID, field.TypeString))
	_spec.From = sq.sql
