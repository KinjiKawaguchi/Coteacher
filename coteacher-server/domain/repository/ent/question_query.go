...
type QuestionQuery struct {
	...
	withForm           *FormQuery
	withTextQuestion   *TextQuestionQuery
	withQuestionOption *QuestionOptionQuery
...
func (qq *QuestionQuery) QueryForm() *FormQuery {
	query := (&FormClient{config: qq.config}).Query()
...
// QueryTextQuestion chains the current query on the "textQuestion" edge.
func (qq *QuestionQuery) QueryTextQuestion() *TextQuestionQuery {
	query := (&TextQuestionClient{config: qq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := qq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryQuestionOption chains the current query on the "questionOption" edge.
func (qq *QuestionQuery) QueryQuestionOption() *QuestionOptionQuery {
	query := (&QuestionOptionClient{config: qq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := qq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (qq *QuestionQuery) QueryAnswer() *AnswerQuery {
...
func (qq *QuestionQuery) Clone() *QuestionQuery {
	...
	return &QuestionQuery{
		...
		withForm:           qq.withForm.Clone(),
		withTextQuestion:   qq.withTextQuestion.Clone(),
		withQuestionOption: qq.withQuestionOption.Clone(),
...
func (qq *QuestionQuery) WithForm(opts ...func(*FormQuery)) *QuestionQuery {
	query := (&FormClient{config: qq.config}).Query()
...
// WithTextQuestion tells the query-builder to eager-load the nodes that are connected to
// the "textQuestion" edge. The optional arguments are used to configure the query builder of the edge.
func (qq *QuestionQuery) WithTextQuestion(opts ...func(*TextQuestionQuery)) *QuestionQuery {
	query := (&TextQuestionClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	qq.withTextQuestion = query
	return qq
}

// WithQuestionOption tells the query-builder to eager-load the nodes that are connected to
// the "questionOption" edge. The optional arguments are used to configure the query builder of the edge.
func (qq *QuestionQuery) WithQuestionOption(opts ...func(*QuestionOptionQuery)) *QuestionQuery {
	query := (&QuestionOptionClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	qq.withQuestionOption = query
	return qq
}
...
func (qq *QuestionQuery) WithAnswer(opts ...func(*AnswerQuery)) *QuestionQuery {
	query := (&AnswerClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (qq *QuestionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Question, error) {
	var (
		...
		loadedTypes = [4]bool{
			qq.withForm != nil,
			qq.withTextQuestion != nil,
			qq.withQuestionOption != nil,
			qq.withAnswer != nil,
		}
	)
	...
	if query := qq.withForm; query != nil {
		if err := qq.loadForm(ctx, query, nodes, nil,
			func(n *Question, e *Form) { n.Edges.Form = e }); err != nil {
			return nil, err
		}
	}
	if query := qq.withTextQuestion; query != nil {
		if err := qq.loadTextQuestion(ctx, query, nodes,
			func(n *Question) { n.Edges.TextQuestion = []*TextQuestion{} },
			func(n *Question, e *TextQuestion) { n.Edges.TextQuestion = append(n.Edges.TextQuestion, e) }); err != nil {
			return nil, err
		}
	}
	if query := qq.withQuestionOption; query != nil {
		if err := qq.loadQuestionOption(ctx, query, nodes,
			func(n *Question) { n.Edges.QuestionOption = []*QuestionOption{} },
			func(n *Question, e *QuestionOption) { n.Edges.QuestionOption = append(n.Edges.QuestionOption, e) }); err != nil {
			return nil, err
		}
	}
	if query := qq.withAnswer; query != nil {
		if err := qq.loadAnswer(ctx, query, nodes,
			func(n *Question) { n.Edges.Answer = []*Answer{} },
			func(n *Question, e *Answer) { n.Edges.Answer = append(n.Edges.Answer, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (qq *QuestionQuery) loadForm(ctx context.Context, query *FormQuery, nodes []*Question, init func(*Question), assign func(*Question, *Form)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "form_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (qq *QuestionQuery) loadTextQuestion(ctx context.Context, query *TextQuestionQuery, nodes []*Question, init func(*Question), assign func(*Question, *TextQuestion)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(textquestion.FieldQuestionID)
	}
	query.Where(predicate.TextQuestion(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(question.TextQuestionColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (qq *QuestionQuery) loadQuestionOption(ctx context.Context, query *QuestionOptionQuery, nodes []*Question, init func(*Question), assign func(*Question, *QuestionOption)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Question)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(questionoption.FieldQuestionID)
	}
	query.Where(predicate.QuestionOption(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(question.QuestionOptionColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (qq *QuestionQuery) loadAnswer(ctx context.Context, query *AnswerQuery, nodes []*Question, init func(*Question), assign func(*Question, *Answer)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Question)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Answer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(question.AnswerColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (qq *QuestionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := qq.ctx.Fields; len(fields) > 0 {
		...
		if qq.withForm != nil {
			_spec.Node.AddColumnOnce(question.FieldFormID)
		}
	}
