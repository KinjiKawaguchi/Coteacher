...
func (qoq *QuestionOptionQuery) QuerySelectedOption() *SelectedOptionQuery {
	query := (&SelectedOptionClient{config: qoq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := qoq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (qoq *QuestionOptionQuery) Clone() *QuestionOptionQuery {
	...
	return &QuestionOptionQuery{
		...
		withSelectedOption: qoq.withSelectedOption.Clone(),
...
func (qoq *QuestionOptionQuery) WithSelectedOption(opts ...func(*SelectedOptionQuery)) *QuestionOptionQuery {
	query := (&SelectedOptionClient{config: qoq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	qoq.withSelectedOption = query
	return qoq
}
...
//		GroupBy(questionoption.FieldQuestionID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (qoq *QuestionOptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*QuestionOption, error) {
	...
	if query := qoq.withSelectedOption; query != nil {
		if err := qoq.loadSelectedOption(ctx, query, nodes,
			func(n *QuestionOption) { n.Edges.SelectedOption = []*SelectedOption{} },
			func(n *QuestionOption, e *SelectedOption) { n.Edges.SelectedOption = append(n.Edges.SelectedOption, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (qoq *QuestionOptionQuery) loadSelectedOption(ctx context.Context, query *SelectedOptionQuery, nodes []*QuestionOption, init func(*QuestionOption), assign func(*QuestionOption, *SelectedOption)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(selectedoption.FieldOptionID)
	}
	query.Where(predicate.SelectedOption(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(questionoption.SelectedOptionColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
