...
type AnswerQuery struct {
	...
	withSelectedOption *SelectedOptionQuery
...
func (aq *AnswerQuery) QueryResponse() *ResponseQuery {
...
// QuerySelectedOption chains the current query on the "selectedOption" edge.
func (aq *AnswerQuery) QuerySelectedOption() *SelectedOptionQuery {
	query := (&SelectedOptionClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AnswerQuery) Clone() *AnswerQuery {
	...
	return &AnswerQuery{
		...
		withResponse:       aq.withResponse.Clone(),
		withSelectedOption: aq.withSelectedOption.Clone(),
...
func (aq *AnswerQuery) WithResponse(opts ...func(*ResponseQuery)) *AnswerQuery {
	query := (&ResponseClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withResponse = query
	return aq
}

// WithSelectedOption tells the query-builder to eager-load the nodes that are connected to
// the "selectedOption" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AnswerQuery) WithSelectedOption(opts ...func(*SelectedOptionQuery)) *AnswerQuery {
	query := (&SelectedOptionClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withSelectedOption = query
	return aq
}
...
func (aq *AnswerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Answer, error) {
	...
	if query := aq.withResponse; query != nil {
		if err := aq.loadResponse(ctx, query, nodes, nil,
			func(n *Answer, e *Response) { n.Edges.Response = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withSelectedOption; query != nil {
		if err := aq.loadSelectedOption(ctx, query, nodes,
			func(n *Answer) { n.Edges.SelectedOption = []*SelectedOption{} },
			func(n *Answer, e *SelectedOption) { n.Edges.SelectedOption = append(n.Edges.SelectedOption, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AnswerQuery) loadResponse(ctx context.Context, query *ResponseQuery, nodes []*Answer, init func(*Answer), assign func(*Answer, *Response)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Answer)
	for i := range nodes {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "response_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AnswerQuery) loadSelectedOption(ctx context.Context, query *SelectedOptionQuery, nodes []*Answer, init func(*Answer), assign func(*Answer, *SelectedOption)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(selectedoption.FieldAnswerID)
	}
	query.Where(predicate.SelectedOption(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(answer.SelectedOptionColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "answer_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (aq *AnswerQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := aq.ctx.Fields; len(fields) > 0 {
		...
		if aq.withResponse != nil {
			_spec.Node.AddColumnOnce(answer.FieldResponseID)
		}
	}
