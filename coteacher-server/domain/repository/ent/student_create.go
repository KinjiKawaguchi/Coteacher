...
func (sc *StudentCreate) AddStudentClassIDs(ids ...int) *StudentCreate {
	sc.mutation.AddStudentClassIDs(ids...)
	return sc
}

// AddStudentClasses adds the "studentClasses" edges to the StudentClass entity.
func (sc *StudentCreate) AddStudentClasses(s ...*StudentClass) *StudentCreate {
	ids := make([]int, len(s))
	for i := range s {
		ids[i] = s[i].ID
	}
...
func (sc *StudentCreate) createSpec() (*Student, *sqlgraph.CreateSpec) {
	...
	if nodes := sc.mutation.StudentClassesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
